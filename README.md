...
| 文档 | 用途 | 权威性 |
|---|---|---|
| [策略算法框架详细设计 v6](docs/策略算法框架详细设计-v6.md) | 当前代码事实、算法审计、未退出项、目标架构和下一开发路线 | 唯一在研设计 |
...
当前默认链路仍使用单结算和价格接受者单 BESS 运行。Award 约束、资源实测、可选多资源日内与 synthetic-only 工程验收已接线，但多资源尚未成为正式结算主路径，双结算仍是规则库，市场仿真仍为离线原型。完整事实、未退出项和下一批次见 [v6](docs/策略算法框架详细设计-v6.md)。
...
完整仓库验证快照与生产边界见 [v6 §8](docs/策略算法框架详细设计-v6.md)。2026-08-08 的全仓命令 `env -u PYTHONPATH UV_CACHE_DIR=.uv_cache uv run pytest -q -W error::RuntimeWarning` 结果为 `789 passed, 4 skipped, 5 deselected, 11 warnings`；这不是永久测试数量，也不证明生产效果。

## 当前实现状态

- 单结算样例主链已形成可运行闭环，决策全程经事件链可追踪。
- 预测时间契约、场景概率、BESS 运行计划、结算分项和事件链完整性已有活动测试保护。
- 市场配置、结算和价格角色已插件化；头寸、运行和报价策略尚未成为完整的 MarketProfile 可替换策略；规则驱动 Bid→Award→资源履约→结算闭环属于 v6 后续批次。
...
- 当前代码事实、实施状态、验证快照或下一批次变化时更新 [v6](docs/策略算法框架详细设计-v6.md)。
