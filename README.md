...
当前默认链路使用单结算。市场模式经 `MarketMode` 协议在入口选择注入；双结算具备规则插件但尚无完整主链用例。价格角色和日内新 forecast vintage 已接线；多资源、对账、验收和市场仿真仍是待接线算法核。完整事实与下一批次见 [v5 §3、§14](docs/策略算法框架详细设计-v5.md#3-当前实现基线与缺口)。
