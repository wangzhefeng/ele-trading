...
`ele-trading` 是一个面向虚拟电厂、电力现货交易、储能调度与风光储测算研究的研究型原型项目。它把调研文档中的"数据/气象 -> 预测/资源模拟 -> 场景 -> 优化 -> 控制 -> 结算 -> 回测/测算"闭环拆成可以持续扩展的工程模块，而不是停留在一次性脚本。
...
3. 给风光功率预测、偏差考核优化、风光储联合调度与容量规划测算预留稳定接口。
...
- `SolarPowerForecaster`：支持 `harmonic` 与 `physics` 两种模式的光伏功率预测
- `WindPowerForecaster`：支持 `statistical` 与 `physics` 两种模式的风电功率预测
- `RenewableForecastStub`：保留向后兼容的占位接口
...
- 光伏物理模拟（`pvlib`）与风电物理模拟（`windpowerlib`）
- 光伏谐波预测与风电 AR/气象预测
- 风光储容量规划优化与全年时序测算
- 风光储一体化演示入口（`app/run_wind_solar_storage.py`）
- 样例价格数据与储能参数
...
涉及的主要依赖包括 `pulp`、`pyomo`、`scipy`、`rainflow`、`pvlib`、`windpowerlib`。  
...
项目根目录下的 `docs/research/` 已整理多份调研与方案文档，包括：

- 电力市场交易调研文档 v1/v2
- 风光储测算算法方案 v1/v2
- 风光储测算过程配图与补充材料

这些文档用于解释工程设计来源与算法背景，不应替代代码层面的实际可运行状态判断。
