...
| `capacity_planning.yaml` | `run_wind_pv_bess_capacity_planning_2.py` | 风光储联合容量规划三场景演示、约束、搜索步长和成本参数 |
...
| `wind_pv_bess_capacity_planning.yaml` | `run_wind_pv_bess_capacity_planning_1.py` | Wind+PV+BESS 容量规划、PV 搜索、BESS 搜索和能量门槛检查 |
...
- 市场规则参数放入 `market_*.yaml`，例如偏差死区、分层阈值、价格上下限。
- 设备物理参数放入对应设备或调度配置，例如 `storage.yaml`、`*_dispatch.yaml`、`*_capacity_planning.yaml`。
...
uv run python app/run_wind_pv_bess_capacity_planning_2.py
