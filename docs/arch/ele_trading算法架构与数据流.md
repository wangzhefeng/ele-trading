...
> 权威关系：设计意图与业务口径以 `docs/策略算法框架详细设计-v2.md`（下称 v2 设计）为准；项目硬约束以根目录 `AGENTS.md` 为准。本文只做实现现状的事实性描述，不重复 v2 设计论证；两者冲突时以 v2 设计 + 代码为准并修正本文。
> 范围：`data_provider/`、`forecasting/`、`scenario/`、`optimization/`、`trading/`、`utils/` 的活动代码。归档用户侧包 `user_side_dispatch/` 与 `trading/todo/`（v1 双结算）不在本文范围。
...
- 市场参数只允许来自 `configs/trading/market_mengxi.yaml` → `trading.config_loader.load_market_config()`，代码中不得硬编码罚款系数、价格限幅等。
...
`configs/trading/market_mengxi.yaml` → `load_market_config()` → `MarketConfig`（YAML 叶子与字段**严格一一对应**，多/少字段都失败）。区段与字段：
