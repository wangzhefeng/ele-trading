...
class EssSimulationModel:
    ...
    def one_step(self, time_lag, demand_load, command, soc):
        ...
        # 充电过程
            ...
            # print(f"inner_energy_vari: {inner_energy_vari}")
            # 储能器容量限制
    ...
    def simulation_process(self, demand_load, es_strategy, last_soc=0):
        # charge
        es_charge_list = []
        es_charge_time_list = []
        # soc
        ...
        for index, row in demand_load.iterrows():
            ...
            if time_i:
                ...
                time_diff_hour = time_diff.total_seconds() / (60 * 60)  # 5/60 hour
            ...
            # 更新 time_i 为当前时间索引
            time_i = index
            # 更新 demand_load_i, es_strategy_i
        ...
        es_charge_time_list.append(time_i)
        # soc
        es_soc_list.append(es_soc_i)
        es_soc_time_list.append(time_i + time_diff)
        # ------------------------------
        # 结果组织
        # ------------------------------
        # charge
        es_charge_df = pd.DataFrame({
            "value": es_charge_list
        ...
        # soc
        es_soc_df = pd.DataFrame({
            "value": es_soc_list
        }, index = es_soc_time_list)
