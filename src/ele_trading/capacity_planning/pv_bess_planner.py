...
- 平移充电模式 (enable_shift=True): 允许 PV 不足时从电网少量充电（需配合电价信号）
...
class ShiftPolicy:
    ...
    shift_max_frac_of_pv: float = 0.30
...
def _simulate_surplus_shift(
    ...
    if cap_kwh <= 0:
        served = np.minimum(pv_kw, load_kw)
        curtail = np.maximum(pv_kw - served, 0.0)
        soc = np.full(n, cfg.soc_init, dtype=float)
        charge = np.zeros(n, dtype=float)
        discharge = np.zeros(n, dtype=float)
        return _post_metrics(served, load_kw, pv_kw, charge, discharge, soc, curtail, dt_h, cap_kwh)
    ...
    for t in range(n):
        ...
        P = float(max(0.0, pv_kw[t]))

        served_direct = min(L, P)
        surplus = max(P - L, 0.0)
        deficit = max(L - P, 0.0)
...
def _simulate_shift(
    ...
    """
    平移充电模式：
    - 允许 PV < Load 时抽取部分 PV 充电（通过 lookahead 预判未来缺口）
    - 禁止电网充电
    """
    n = len(load_kw)
    ...
    for t in range(n):
        ...
        if ch_max > 0 and P > 0:
            ...
            if future_def > 0.5 * L * (t2 - t) and soc_ratio < 0.7:
                ch_plan = min(ch_max, policy.shift_max_frac_of_pv * P)

        # 2) PV 分配：先预留 ch_plan，再供负荷
        P_after_ch = max(0.0, P - ch_plan)
        serve_from_pv = min(L, P_after_ch)

        # 3) 电池放电补缺口
        deficit = L - serve_from_pv
        dis_out = min(dis_max_out, max(0.0, deficit))
        served_t = serve_from_pv + dis_out

        # 4) 富余 PV 继续充电
        surplus = max(0.0, P - serve_from_pv - ch_plan)
        ch_extra = min(max(0.0, ch_max - ch_plan), surplus)
        ch_in = ch_plan + ch_extra

        # 5) 弃电
        curtail_t = max(0.0, P - serve_from_pv - ch_in)
    ...
    if cfg.enforce_terminal_soc:
        if abs(soc[-1] - cfg.soc_init) > 0.02:
            res = _post_metrics(served, load_kw, pv_kw, charge, discharge, soc, curtail, dt_h, cap_kwh)
...
def simulate_dispatch(
    ...
    if cfg.shift_policy.enable_shift:
        return _simulate_shift(load_kw, pv_kw, dt_h, cap_kwh, cfg, cfg.shift_policy)
    else:
        return _simulate_surplus_shift(load_kw, pv_kw, dt_h, cap_kwh, cfg)
...
def _post_metrics(
    ...
    self_consumption = (e_served / e_pv) if e_pv > 0 else 0.0
...
def check_feasibility_upper_bound(
    ...
    r_inf = simulate_dispatch(load_kw, pv_kw, dt_h, cap_kwh=1e9, cfg=cfg)
    return {
...
# 可行性判断
# ============================================================
def is_feasible(res: dict[str, Any], cfg: PVBESSPlanConfig) -> bool:
    m = res["metrics"]
...
def find_min_capacity_bisect(
    ...
    while hi <= cfg.cap_max_mwh * 1000.0 + 1e-9:
        res = simulate_dispatch(load_kw, pv_kw, dt_h, hi, cfg)
    ...
    while (hi - lo) > tol_kwh:
        mid = (lo + hi) / 2.0
        res = simulate_dispatch(load_kw, pv_kw, dt_h, mid, cfg)
        if is_feasible(res, cfg):
            best = res
            hi = mid
        else:
    ...
    best = simulate_dispatch(load_kw, pv_kw, dt_h, cap_final_kwh, cfg)
...
def plan_pv_bess_system(
    ...
    try:
        ...
        # 快速诊断
        diag = quick_feasibility_diagnose(load_kw, pv_kw, dt_h, cfg)

        # 二分求最小容量
        result = find_min_capacity_bisect(load_kw, pv_kw, dt_h, cfg)
...
def calc_monthly_pv_metrics(
    ...
    df = df[[load_col, pv_col]].copy()

    dt_hours = (
        df.index.to_series().diff().dt.total_seconds().median() / 3600
    )

    df["used_kW"] = np.minimum(df[pv_col], df[load_col])
    df["pv_kWh"] = df[pv_col] * dt_hours
    df["load_kWh"] = df[load_col] * dt_hours
    df["used_kWh"] = df["used_kW"] * dt_hours

    monthly = df.resample("M").sum()

    monthly["curtail_kWh"] = monthly["pv_kWh"] - monthly["used_kWh"]
    monthly["pv_consumption_rate"] = monthly["used_kWh"] / monthly["pv_kWh"]
    monthly["load_coverage_rate"] = monthly["used_kWh"] / monthly["load_kWh"]

    result = monthly[[
        "pv_kWh", "used_kWh", "curtail_kWh", "load_kWh",
        "pv_consumption_rate", "load_coverage_rate",
    ]].copy()

    result.columns = [
        "光伏发电量(kWh)", "光伏消纳电量(kWh)", "弃光电量(kWh)",
        "用电量(kWh)", "光伏消纳率", "负荷覆盖率",
    ]
...
def plot_capacity_curve(
    ...
    Args:
        df: 包含 Load_kW 和 PV_kW 列的 DataFrame
        dt_h: 时间步长 (h)
        cfg: PV+BESS 规划配置
    ...
    selfs = []
    for c in caps:
        r = simulate_dispatch(load_kw, pv_kw, dt_h, float(c) * 1000.0, cfg)
        covs.append(r["metrics"]["load_coverage"])
        selfs.append(r["metrics"]["self_consumption"])

    plt.figure()
    plt.plot(caps, covs, marker="o", label="Load coverage")
    plt.plot(caps, selfs, marker="o", label="Self consumption")
    plt.axhline(cfg.min_load_coverage, linestyle="--", alpha=0.5, label=f"Coverage target ({cfg.min_load_coverage:.0%})")
    plt.axhline(cfg.min_self_consumption, linestyle="--", alpha=0.5, label=f"Self-consumption target ({cfg.min_self_consumption:.0%})")
