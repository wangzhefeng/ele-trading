...
量价结算 C、差价结算 C2、日前偏差考核 Cpen_dayah、中长期月度回收
Cpen_long。公式迁移自 v1.3 设计（§10.1），逐行不变。
结算时段聚合 ``aggregate_to_settle_periods`` 统一由 ``markets.shared`` 提供。
"""
...
from ele_trading.markets.shared import aggregate_to_settle_periods
