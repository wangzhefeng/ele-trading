...
def run_user_side_renewable_bess_dispatch(
    ...
    """Run user-side renewable + BESS dispatch as a MILP."""
    _validate_input(dispatch_input)

    bess = dispatch_input.bess
    T = range(len(dispatch_input.timestamps))
    model = LpProblem("user_side_renewable_bess_dispatch", LpMinimize)

    renewable_to_load = {t: LpVariable(f"renewable_to_load_{t}", lowBound=0) for t in T}
    renewable_to_bess = {t: LpVariable(f"renewable_to_bess_{t}", lowBound=0) for t in T}
    renewable_to_grid = {t: LpVariable(f"renewable_to_grid_{t}", lowBound=0) for t in T}
    renewable_curtailment = {t: LpVariable(f"renewable_curtailment_{t}", lowBound=0) for t in T}
    ...
    for t in T:
        ...
        model += (
            renewable_to_load[t] + discharge[t] + grid_to_load[t]
            == dispatch_input.load_forecast[t]
        )
        model += charge[t] == renewable_to_bess[t] + grid_to_bess[t]
        ...
        if not dispatch_input.export.allow_export:
            model += renewable_to_grid[t] == 0
        if dispatch_input.export.export_limit is not None:
            model += renewable_to_grid[t] <= dispatch_input.export.export_limit
    ...
    check_pulp_status(model, "user-side renewable bess dispatch")

    renewable_to_load_values = [clean_value(value(renewable_to_load[t])) for t in T]
    renewable_to_bess_values = [clean_value(value(renewable_to_bess[t])) for t in T]
    renewable_to_grid_values = [clean_value(value(renewable_to_grid[t])) for t in T]
    renewable_curtailment_values = [clean_value(value(renewable_curtailment[t])) for t in T]
    ...
    sell_revenue = sum(
        dispatch_input.export.sell_price
        * renewable_to_grid_values[t]
        * dispatch_input.step_hours
        for t in T
    )
    curtailment_cost = sum(
        dispatch_input.export.curtailment_cost_rate
        * renewable_curtailment_values[t]
        * dispatch_input.step_hours
        for t in T
    )
    cycle_cost = sum(
    ...
    return UserSideRenewableBESSDispatchResult(
        renewable_to_load=renewable_to_load_values,
        renewable_to_bess=renewable_to_bess_values,
        renewable_to_grid=renewable_to_grid_values,
        renewable_curtailment=renewable_curtailment_values,
        ...
        constraint_violations=_constraint_violations(
            dispatch_input,
            renewable_to_load_values,
            renewable_to_bess_values,
            renewable_to_grid_values,
            renewable_curtailment_values,
            grid_to_load_values,
            grid_to_bess_values,
            charge_values,
            discharge_values,
...
def _validate_input(dispatch_input: UserSideRenewableBESSDispatchInput) -> None:
...
def _policy_objective(
    ...
    return (
        ...
        * lpSum(renewable_to_bess[t] * dispatch_input.step_hours for t in T)
        - policy.pv_to_load_reward_rate
        * lpSum(renewable_to_load[t] * dispatch_input.step_hours for t in T)
        + policy.pv_export_penalty_rate
        * lpSum(renewable_to_grid[t] * dispatch_input.step_hours for t in T)
    )
...
def _constraint_violations(
    ...
    violations = {
        ...
        "charge_balance": max(
            abs(charge_values[t] - renewable_to_bess[t] - grid_to_bess[t])
            for t in range(len(dispatch_input.timestamps))
        ),
