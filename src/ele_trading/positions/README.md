...
当前头寸层消费共享 `MarketConfig`，并保留透明走廊与可选 CVaR 优化。它尚未成为 MarketMode 可替换的 `PositionPolicy`，也没有真实订单簿/成交校准；报价—成交链与标定要求见 [v5 V5-8～V5-10](../../../docs/策略算法框架详细设计-v5.md#143-v5-8报价成交履约证据链下一开发批次)。
