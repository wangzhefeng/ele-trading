...
from .contracts import MarketConfig
...
def load_market_config(path: str | Path) -> MarketConfig:
    ...
    field_map = {
        ...
        "m_long": "m_long",
        "scenario_deviation_cost_positive": "two_stage_scenario_deviation_cost_positive",
        "scenario_deviation_cost_negative": "two_stage_scenario_deviation_cost_negative",
        "gap": "gap",
        "bias_k": "bias_k",
...
def _validate_config(config: MarketConfig) -> None:
    ...
    for yaml_name, value in {
        "two_stage.scenario_deviation_cost_positive": (
            config.two_stage_scenario_deviation_cost_positive
        ),
        "two_stage.scenario_deviation_cost_negative": (
            config.two_stage_scenario_deviation_cost_negative
        ),
    }.items():
        if (
            ...
            raise ValueError(
                f"{yaml_name} must be finite and non-negative, got {value}"
            )
