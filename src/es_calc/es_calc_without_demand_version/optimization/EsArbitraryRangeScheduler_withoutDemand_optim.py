...
class EsArbitraryRangeScheduler_withoutDemand:
    ...
    def modeling2solving(self):
        ...
        # profit = time_ratio * (
        #     e_c_in_agg_vec + e_c_out_agg_vec
        # ) @ p - self.max_demand_price * cp.max(d - (e_c_in_agg_vec + e_c_out_agg_vec))
        ...
        # 总功率小于变压器容量
        # constraints += [d - e_c_in_agg_vec <= self.transform_capacity_list[0]]
        # 储能系统每个时段的充放电功率限制
        ...
        # 对电量损耗的保底电量限制, 此条限制在滚动策略中无法保证满足，建议在EMS中进行设置
