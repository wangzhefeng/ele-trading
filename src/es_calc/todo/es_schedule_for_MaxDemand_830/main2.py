...
class ModelMainClass:#(BaseModelMainClass):
    ...
    def optimization(self, input_data, model_cfgs, devices_info, month_num):
        ...
        for ratio in range(model_cfgs["ratio_min"], model_cfgs["ratio_max"], 10):
            ...
            # TODO results save
            # --------------
            # save_result_df.to_csv(f"./data/{model_cfgs['exp_name']}/route_A_05/opt_result/ratio_experiment_dod97/schedule_result_fixline_up{ratio}-2.csv", encoding="utf-8", index=False)
            # save_demand_load_df.to_csv(f"./data/{model_cfgs['exp_name']}/route_A_05/opt_result/demand_load-2.csv")
            # save_ele_price_df.to_csv(f"./data/{exp_name}/{node_name}/opt_result/ele_price.csv")
            logger.info(f"每月策略调度模型::month_num-ratio: {month_num:02d}_{ratio} end...")
            logger.info(f"{'='*80}")
    ...
    def run(self, input_data: Dict, model_cfgs: Dict):
        ...
        mp_input_list_month = [
            (input_data, model_cfgs, devices_info, month_num) 
            for month_num in [5, 7, 8, 9]
        ]
        with mp.Pool(processes=8) as pool:
            # policy optimization
            pool.starmap(self.optimization, mp_input_list_month)
            # simulation
            res = pool.starmap(self.simulation, mp_input_list_month)\
        
        for item in res:
            max_break_ratio = item[0]
            strategy_df = item[1]
    ...
    input_data = {
        ...
        "future": {
            "demand_load": {
                ...
                "month_05": pd.read_csv(f"./data/{model_cfgs['exp_name']}/route_A_05/opt_result/demand_load.csv"),
