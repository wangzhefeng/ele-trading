...
class EsArbitraryRangeScheduler_withMaxDemand_fixedLine:
    # TODO 拆分设备参数和运行数据
    def __init__(self, 
...
def one_process(es_scale, route_num_str, revenue_discount):
    ...
    for time_pair in validation_day_list:
        ...
        strategy_df = opt_list[0]
        strategy_df.rename(columns={"power_opt": "value"}, inplace=True)

        sim_demand_load_df = copy.deepcopy(step_demand_load_df)
        sim_demand_load_df.set_index('time', inplace=True)
        sim_ele_price_df = copy.deepcopy(step_ele_price_df)
        sim_ele_price_df.set_index('time', inplace=True)

        simulation_model = EssSimulationModel(es_info)
        es_charge_df, es_soc_df, total_load_df = simulation_model.simulation_process(sim_demand_load_df, strategy_df, 0)
        origin_balance, opt_balance = simulation_model.revenue_calculation(sim_demand_load_df, es_charge_df, sim_ele_price_df, max_demand_price)
        ori_max_demand = demand_load_df["value"].max()
        opt_max_demand = total_load_df["total_load"].max()
        max_demand_rise_cost = (opt_max_demand - ori_max_demand) * max_demand_price
        revenue = origin_balance - opt_balance - max_demand_rise_cost
        max_demand_control_line = opt_max_demand + revenue * (1 - revenue_discount) / max_demand_price

        scheduler_model = EsArbitraryRangeScheduler_withMaxDemand_fixedLine(step_demand_load_df["time"].to_list(), 
                                                                            step_demand_load_df["value"].to_list(), 
                                                                            step_ele_price_df["value"].to_list(), 
                                                                            step_ele_price_df["type"].to_list(),
                                                                            devices_info,
                                                                            [0],
                                                                            max_demand_control_line)
        opt_list = scheduler_model.run()
...
if __name__ == '__main__':
    ...
    es_scale_list = list(range(26000, 27000, 1000))
    route_list = [("fluctuation10", 0.89), ("fluctuation20", 0.91), ("fluctuation30", 0.92)]
    mp_input_list = [(x, y[0], y[1]) for x in es_scale_list for y in route_list]
    mp_result_list = []
    ...
    for result_i in mp_result_list:
        ...
        result_i[2].to_csv(f"./data/{exp_name}/{node_name}/opt_result/es_scale_experiment/schedule_result_scale_{es_scale}.csv")