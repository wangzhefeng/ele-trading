...
class EsArbitraryRangeScheduler_withMaxDemand:
    ...
    def modeling2solving(self):
        ...
        #     elif self.ele_types[j] == "峰":False
        #         constraints += [e_c_in_agg_vec[j] == 0]
        #     elif self.ele_types[j] == "尖峰":
        #         constraints += [e_c_in_agg_vec[j] == 0]True
        #     elif self.ele_types[j] == "平":
        #         constraints += [e_c_out_agg_vec[j] == 0]

        prob = cp.Problem(obj, constraints)
        result = prob.solve(verbose = False, solver = cp.SCIP)
...
if __name__ == '__main__':
    ...
    for result_i in mp_result_list:
        ...
        result_dir = Path(f"./data/{exp_name}/{node_name}/opt_result/es_scale_experiment_optim/")
        result_dir.mkdir(parents=True, exist_ok=True)
        result_i[2].to_csv(result_dir.joinpath(f"schedule_result_scale_{es_scale}.csv"))
