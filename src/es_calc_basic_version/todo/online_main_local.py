...
from model.model_packages.ProfitSimulation_WithMaxDemand_ByDay.models.optimization.EsArbitraryRangeScheduler_withMaxDemand_optim_local import (
...
#     EssSimulationModel
# )
from model.model_packages.ProfitSimulation_WithMaxDemand_ByDay.utils.time_process import generate_day_pairs
...
class ModelMainClass(BaseModelMainClass):
    ...
    def simulation(self, 
        ...
        time_point_list = generate_day_pairs(
            start_time=pd.to_datetime("2025-11-30 22:00:00"), 
            end_time=pd.to_datetime("2025-12-25 22:00:00")
        )
        for time_point in time_point_list:
            s_time, e_time = time_point[0], time_point[1]
            logger.info(f"s_time: {s_time}, e_time: {e_time}")
            total_load_df_i = total_load_df.loc[
                (total_load_df.index >= pd.to_datetime("2025-12-01 00:00:00")) & 
                (total_load_df.index < pd.to_datetime("2026-01-01 00:00:00")), :
            ]
            demand_load_df_i = demand_load_df.loc[(demand_load_df.index >= s_time) & (demand_load_df.index < e_time), :]
            # with pd.option_context('display.max_rows', None, 'display.max_columns', None):
            #     logger.info(f"demand_load_df_i: \n{demand_load_df_i}")
            es_charge_df_i = es_charge_df.loc[(es_charge_df.index >= s_time) & (es_charge_df.index < e_time), :]
            ele_price_df_i = ele_price_df.loc[(ele_price_df.index >= s_time) & (ele_price_df.index < e_time), :]
            
            origin_balance, opt_balance = simulation_model.revenue_calculation(
                demand_load = demand_load_df_i, 
                es_load = es_charge_df_i, 
                ele_price = ele_price_df_i, 
                max_demand_price = max_demand_price,
            )
            # ------------------------------
            # cost
            # ------------------------------
            # 用电总量
            total_energy = demand_load_df_i["value"].sum()
            
            # 需量电费成本
            opt_max_demand_load_list, ori_max_demand_load_list = get_monthly_max_load(total_load_df_i)
            logger.info(f"opt_max_demand_load_list: {opt_max_demand_load_list}")
            logger.info(f"ori_max_demand_load_list: {ori_max_demand_load_list}")
            ...
            # 充放电电量、成本（收益）
            es_charge_df_i["price"] = ele_price_df_i["value"]
            es_charge_df_i["balance"] = es_charge_df_i["value"] * es_charge_df_i["price"]
            charge_energy = - es_charge_df_i.loc[es_charge_df_i['value'] < 0, 'value'].sum()
            discharge_energy = es_charge_df_i.loc[es_charge_df_i['value'] > 0, 'value'].sum()
            charge_balance = - es_charge_df_i.loc[es_charge_df_i['balance'] < 0, 'balance'].sum()
            discharge_balance = es_charge_df_i.loc[es_charge_df_i['balance'] > 0, 'balance'].sum()
    ...
    def run(self, model_cfgs: Dict, input_data: Dict, route: str):
        # ------------------------------
        # experiment params
        # ------------------------------
        logger.info(f"{'=' * 100}")
        ...
        devices_info[0]["es_capacity_max"] = 17888 # es_scale * 2
        ...
        strategy_df.to_excel(f"./data/estimate_zhangjiakou/route_{route}/strategy_df_{route}.xlsx")
        logger.info(f"strategy_df: \n{strategy_df}")
        # ------------------------------
        # simulation
        # ------------------------------
        ...
        result.to_csv(f"./data/estimate_zhangjiakou/route_{route}/result_{route}.csv", encoding="utf-8", index=True)
        """
        # ------------------------------
        # 指标计算
        # ------------------------------
...
def main():
    ...
    # ##############################
    input_data = {
        "demand_load": pd.read_csv(f"./data/estimate_zhangjiakou/route_{route}/demand_load.csv"),
        "ele_price": pd.read_csv(f"./data/estimate_zhangjiakou/route_{route}/ele_price.csv"),
    }
    # ##############################
    # model
    # ##############################
    ...
    res = model.run(model_cfgs=model_cfgs, input_data=input_data, route=route)
    # output
