...
class EsArbitraryRangeScheduler_withMaxDemand:
    ...
    def __init__(
        ...
        allocation_group_labels: List[str] | None = None,
        ...
        self.allocation_group_labels = allocation_group_labels
        ...
        if self.allocation_group_labels is not None and len(self.allocation_group_labels) != self.devices_num:
            raise ValueError("allocation_group_labels length must match devices_info length.")
    ...
    def modeling2solving(self):
        ...
        # 园区从上级电网购电以 demand_load.csv 为基线，再叠加储能充电/放电修正。
        ...
        for source_i in range(row):
            ...
            if self.allocation_group_labels is not None:
                for target_j in range(row):
                    if self.allocation_group_labels[source_i] != self.allocation_group_labels[target_j]:
