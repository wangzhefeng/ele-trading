# invest_est_models 复用 capacity_planning 算法分析 TODO

本文档分析 `src/ele_trading/capacity_planning/`、项目根目录 `app/` 和 `configs/` 中已有算法与运行配置，判断哪些能力对继续构建 `src/invest_est_models/` 有用，哪些暂不适合接入。

分析目标不是把旧代码整体迁入 `invest_est_models`，而是识别可以服务以下目标的算法资产：
...
5. `wind_pv_bess_irr_planner.py`：与 `invest_est_models` 当前 V1-V5 最接近的风光储 IRR 目标型容量搜索。
...
暂不适合直接接入的部分主要是历史场景包装、分布式储能、多节点扫描、聚合 IRR 快速扫描和部分旧 planner。它们要么目标与 `invest_est_models` 当前的“投资方/业主双目标测算”不一致，要么依赖特定园区、多变压器、旧数据口径或求解器路径，直接接入会增加复杂度。

## 对 invest_est_models 最有用的算法
...
1. `invest_est_models` 当前把风光资源作为外部 CSV 输入，尚未实现资源仿真。
...
5. 输出 `SimulationResult.power_series`，单位 kW，能较容易转为 `invest_est_models` 当前资源 CSV 合同：
...
1. 不把全部实现复制进 `invest_est_models`。
2. 先在 `invest_est_models` 新增资源适配层，例如：
...
1. `invest_est_models` 当前 `BESSConfig` 已有功率、容量、效率、SOC 边界，但物理语义分散在配置和调度函数里。
...
1. 给 `invest_est_models` 增加内部物理合同或 adapter。
...
1. 当前 `invest_est_models/dispatch/rule_based.py` 可以运行，但输出表是 DataFrame，缺少独立的调度结果合同。
2. `canonical_dispatch()` 产出 `DispatchSimulationResult`，字段更适合审计：
   ...
   - `monthly_summary`
   - `annual_summary`
3. 它强调“年度汇总必须由月度汇总得到”，这比当前 `invest_est_models` 直接 DataFrame 聚合更适合作为长期主链。
4. 它不允许电网充电，适合作为“自发自用/消纳型”物理基线。

与当前实现差异：

1. `invest_est_models` 当前允许电网低价充电，由 `allow_grid_charge` 控制。
2. `canonical_dispatch()` 当前默认 `grid_charge_kwh=0`，更偏风光余电消纳。
3. `invest_est_models` 当前 PPA 电量包括 `renewable_to_load + charge_from_renewable`；canonical 中 green_used 是 `direct_used + discharge`，口径不同，需要谨慎对齐。
...
1. `invest_est_models` 当前已经有 `settlement/monthly.py`，但结构偏 DataFrame 汇总。
2. `capacity_planning` 的 `MonthlySettlementResult` 明确把月度结算作为财务输入来源：
   ...
   - `savings_ratio`
   - `net_load_peak_kw`
3. 它支持 `Tariff` 和逐时 TOU 价格，能补强当前 `invest_est_models` 中电价模型较弱的问题。
...
1. `invest_est_models/finance/irr.py` 当前已实现 CAPEX、年度现金流、IRR、NPV、回收期、PPA 反求。
...
1. 短期不要替换 `invest_est_models/finance/irr.py`。
...
1. 这是与 `invest_est_models` 当前 V1-V5 最接近的已有算法。
2. 它已经包含：
   - 风/光/储三维容量网格。
   - canonical dispatch。
   - monthly settlement。
...
3. 当前 `invest_est_models` 已实现 V1-V5 的粗网格搜索，但诊断和结算口径还没有这个 planner 完整。
...
3. 先把配置结构和诊断字段映射到 `invest_est_models` 的 YAML 和 CSV 输出。
...
1. `invest_est_models` 当前规则调度通过 `price_type` 控制低价充电和高价放电。
...
3. 当前 `invest_est_models` 已有 `price_type`，是否改成价格阈值需要业务确认。
...
1. `app/README.md` 明确入口脚本只做配置解析、样例数据组装和调用算法，不应堆算法逻辑。这个边界与 `invest_est_models/app/` 当前设计一致。
2. `run_wind_pv_bess_irr_planning.py` 已明确：
   - 正式测算必须显式传 `--data-dir`。
   - demo 输入必须显式传 `--demo`。
   - 输出 CSV 使用中文表头 + 稳定英文机器字段。
3. `configs/README.md` 对配置目录职责有明确约束：配置只描述参数、路径和开关，算法逻辑放在源码中。
4. `wind_pv_bess_irr_planning.yaml` 是比 `invest_est_models/configs/v*_demo.yaml` 更完整的场景模板，包含资源仿真、价格、约束、容量、搜索、BESS、成本和 resource_tuning。

建议接入方式：

1. `invest_est_models/app/` 继续只做编排，不新增算法逻辑。
...
2. 内联贪心仿真口径与 `invest_est_models` 当前调度/结算口径不同。
...
3. 在 `invest_est_models/capacity_search/` 增加后续 TODO：
...
1. 可改造成 `invest_est_models` 的输入诊断模块。
...
1. 后续作为 `invest_est_models` 的 sensitivity/diagnostics 扩展。
2. 不进入当前 V1-V5 主流程。
...
1. 面向多变压器、多储能柜、标准机柜组合，不是当前 `invest_est_models` 的风光储投资测算主目标。
...
3. 容易引入节点级复杂度，干扰 `invest_est_models` 的主链路稳定。
...
2. 当前 `invest_est_models` 的核心要求是 8760 或 15 分钟动态平衡、月度结算和财务闭环。
...
1. 面向旧数据链路和历史兼容。
2. 字段、收益口径、数据路径可能与 `invest_est_models` 当前设计不一致。
3. 直接接入会带来口径混乱。
...
2. 与 `invest_est_models` 的投资测算主链不是同一层。
...
```text
把 capacity_planning/resource_simulation 的输出转为 invest_est_models 的 resource_csv。
```
...
1. 不要把 `capacity_planning` 的所有 planner 直接搬进 `invest_est_models`。目标函数和口径不同，会导致重复和混乱。
...
根据“风光资源仿真”部分的分析，下一步不再建立 `capacity_planning` 到 `invest_est_models` 的适配层，而是把已有风光资源仿真算法代码迁移到 `src/invest_est_models/` 内，形成 `invest_est_models` 自身可维护、可运行、可被后续测算链路调用的资源仿真模块。

迁移后的目标能力包括：

1. 在 `invest_est_models` 内直接生成光伏出力时序。
2. 在 `invest_est_models` 内直接生成风电出力时序。
...
6. 迁移后不要求 `invest_est_models` 运行时依赖 `ele_trading.capacity_planning.resource_simulation`。
...
建议在 `src/invest_est_models/` 内新增独立子模块：

```text
src/invest_est_models/
  resource_simulation/
    README.md
    __init__.py
    models.py
...
入口脚本放在现有 `invest_est_models/app/` 下：

```text
src/invest_est_models/app/
...
配置文件放在现有 `invest_est_models/configs/` 下：

```text
src/invest_est_models/configs/
...
1. 将包导入从 `ele_trading.capacity_planning.resource_simulation` 改为 `invest_est_models.resource_simulation`。
2. 将入口脚本的 YAML 读取改为复用 `invest_est_models.config_loader` 或本地轻量 YAML 读取函数。
3. 将输出字段统一为 `time,pv_kw`、`time,wind_kw` 和 `time,pv_kw,wind_kw`。
4. 将 README 示例导入路径改为 `invest_est_models.resource_simulation`。
...
```text
invest_est_models.resource_simulation.weather.fetch_weather_open_meteo
```

已确认采用完全独立方案：

1. 新增 `invest_est_models/resource_simulation/weather.py`。
2. 迁移 `fetch_weather_open_meteo()`、`load_weather_csv()`、`save_weather_csv()` 和最小 `ensure_datetime_column()`。
3. `invest_est_models/resource_simulation/` 及其入口脚本不导入 `ele_trading.*`。
...
当前 `invest_est_models` 的测算链路读取资源 CSV：
...
1. 新建 `src/invest_est_models/resource_simulation/`。
...
```text
PYTHONPATH=src python -m compileall src/invest_est_models/resource_simulation
```
...
5. 给每个变量补充中文注释。
6. 输出路径统一指向 `src/invest_est_models/dataset/resource_simulation/` 或 `src/invest_est_models/results/resource_simulation/`。
...
1. 在 `src/invest_est_models/app/` 内新增 PV/Wind v1/v2 运行脚本。
2. 入口脚本只负责：
   - 读取 YAML。
   - 构造配置对象。
   - 调用资源仿真模块。
...
```text
PYTHONPATH=src python -m invest_est_models.app.run_pv_simulation_v1 --config src/invest_est_models/configs/resource_pv_simulation_v1.yaml
PYTHONPATH=src python -m invest_est_models.app.run_wind_simulation_v1 --config src/invest_est_models/configs/resource_wind_simulation_v1.yaml
```
...
1. 新增 `src/invest_est_models/app/build_resource_profile.py`。
...
```text
PYTHONPATH=src python -m invest_est_models.app.build_resource_profile --config src/invest_est_models/configs/resource_profile_demo.yaml
```
...
1. 增加 `tests/test_invest_est_models_resource_simulation.py`。
...
```text
PYTHONPATH=src pytest tests/test_invest_est_models_resource_simulation.py -q
```
...
1. 更新 `src/invest_est_models/README.md` 的目录结构和数据输入说明。
2. 更新 `src/invest_est_models/PLAN.md` 中“基础输入层/风光资源建模”的实现进度。
...
1. 已确认 `invest_est_models` 需要完全独立，因此最小 Open-Meteo 气象获取能力已迁移到 `resource_simulation/weather.py`。
2. 已确认第一轮迁移包含 PV v1/v2 和 Wind v1/v2。
3. 已确认算法生成结果默认放 `results/resource_simulation/`，可复用样例气象输入放 `dataset/resource_simulation/`。

## 结论

对 `invest_est_models` 最有用的是 `capacity_planning` 中已经沉淀出的“主链路内核”：
...
后续开发应优先把 P0/P1 能力转化为 `invest_est_models` 的独立模块和配置字段，而不是把 `capacity_planning` 作为运行时依赖整体嵌入。
