...
本模块承载 `invest_est_models` 内独立的风光资源仿真算法。代码从原容量规划资源仿真模块迁移而来，但迁移后运行时不再导入主项目旧包路径。
...
入口脚本位于 `src/invest_est_models/app/`：

```text
python -m invest_est_models.app.run_pv_simulation_v1 --config src/invest_est_models/configs/resource_pv_simulation_v1.yaml
python -m invest_est_models.app.run_pv_simulation_v2 --config src/invest_est_models/configs/resource_pv_simulation_v2.yaml
python -m invest_est_models.app.run_wind_simulation_v1 --config src/invest_est_models/configs/resource_wind_simulation_v1.yaml
python -m invest_est_models.app.run_wind_simulation_v2 --config src/invest_est_models/configs/resource_wind_simulation_v2.yaml
python -m invest_est_models.app.build_resource_profile --config src/invest_est_models/configs/resource_profile_demo.yaml
```
