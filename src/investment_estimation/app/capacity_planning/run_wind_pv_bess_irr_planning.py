...
def _build_wind_unit_curve(config: dict, cache_path: Path) -> pd.Series:
    ...
    """
    from investment_estimation.resource_simulation import WindProfileConfig, load_or_build_wind_profile
...
def _build_pv_unit_curve(config: dict, time_index: pd.DatetimeIndex, cache_path: Path) -> pd.Series:
    ...
    如果 cache_path 已存在则直接读取，否则仿真后保存。
    """
    from investment_estimation.resource_simulation import PVProfileConfig, load_or_build_pv_profile
...
def _resolve_data_dir(data_dir: str | Path | None, demo: bool) -> Path:
    """解析数据目录：显式路径优先，否则要求 --demo 使用内置样例。"""
    if data_dir is not None:
        return Path(data_dir)
    if demo:
        return PROJECT_ROOT / "data" / "profit_calc" / "wind_pv_bess" / "v1"
    logger.error("请通过 --data-dir <path> 指定数据目录，或使用 --demo 加载内置样例数据。")
    raise SystemExit(1)


def main(data_dir: str | Path | None = None, demo: bool = False) -> None:
    # 加载配置
    CONFIG_PATH = PACKAGE_ROOT / 'configs' / 'capacity_planning' / 'wind_pv_bess_irr_planning.yaml'
    config = read_yaml(CONFIG_PATH)
    # 数据目录
    data_dir = _resolve_data_dir(data_dir, demo)
    # ------------------------------
    # 1. 加载真实负荷数据
    # ------------------------------
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="IRR 目标型 Wind+PV+BESS 容量规划")
    parser.add_argument("--data-dir", type=str, default=None, help="数据目录（含 demand_load.csv）")
    parser.add_argument("--demo", action="store_true", help="使用仓库内置 demo 数据")
    args = parser.parse_args()

    main(data_dir=args.data_dir, demo=args.demo)
