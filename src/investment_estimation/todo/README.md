# todo/ — 迁移暂存区（老版 capacity_planning）

> **定位**（2026-07 重组）：本目录是**待整合的迁移暂存区**，收纳老版 `src/ele_trading/capacity_planning/` 的全部投资收益测算内容。它**不是最终形态**——后续将与 `investment_estimation` 新版模块（`dispatch/`、`settlement/`、`finance/`、`capacity_search/` 等）合并去重。
>
> - 本目录代码已**自包含**：跨包引用已改写为 `investment_estimation.utils` / `investment_estimation.finance`，不再 import `ele_trading`。
> - 对外聚合导出仍在 `todo/__init__.py`；`investment_estimation` 顶层 `__init__.py` 不导出 `todo/`。
> - 以下正文为老版 `capacity_planning` README 原文，路径与归属描述以本块为准。

---

`capacity_planning`（现 `investment_estimation.todo`）负责容量搜索、场景编排、收益测算和 CSV/表格结果组织。容量规划专用的 BESS 调度内核副本已归入本目录；`optimization/` 仍保留交易/调度侧原始内核。`models/` 是容量规划流程内复用的仿真与搜索 helper。
