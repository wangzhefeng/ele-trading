"""Archived investment-case construction retained for explicit todo use."""
...
from ele_trading.data_provider.quality import align_series_on_timestamp
...
def build_investment_case_dataset(
    ...
    frame["net_load_kw"] = (
        frame["load_kw"] - frame["pv_kw"] - frame["wind_kw"]
    )
    frame["data_quality_flag"] = (
        frame["quality_score"]
        .fillna(1.0)
        .apply(lambda value: "ok" if value >= 0.8 else "degraded")
    )
    return CaseDataset(
        frame=frame,
