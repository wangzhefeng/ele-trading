...
def one_process(ratio, demand_load_df, ele_price_df, month_num, devices_info):
    ...
    for time_pair in validation_day_list:
        ...
        max_demand_control_line_i = step_demand_load_df["value"].mean() * (1 + ratio/1000)
...
if __name__ == '__main__':
    ...
    mp_input_list = [(ratio, demand_load_df, ele_price_df, month_num, devices_info) for ratio in range(100, 500, 10)]
    mp_result_list = []
    ...
    for result_i in mp_result_list:
        ...
        result_i[1].to_csv(f"./data/{exp_name}/{node_name}/opt_result/2stage_ideal_ratio_dod97/schedule_result_fixline_up{save_ratio}.csv")
