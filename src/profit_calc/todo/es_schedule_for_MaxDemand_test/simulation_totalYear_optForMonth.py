...
def one_process(es_scale, route_num_str, month, max_demand_price):
    es_info = {
    ...
    save_range_start = datetime(2023, 1, 1, 0, 0, 0)
    save_range_end = datetime(2023, 2, 1, 0, 0, 0)
    
    node_name = "route_{}".format(route_num_str)
    demand_load_df = pd.read_csv(f"./data/{exp_name}/{node_name}/demand_load_debug_month_{month}.csv")
    ...
    strategy_df = pd.read_csv(f"./data/{exp_name}/{node_name}/opt_result/es_scale_experiment_optim/schedule_result_scale_{es_scale}.csv")
...
if __name__ == '__main__':
    ...
    # 需量价格
    max_demand_price = 48
    mp_input_list = [(x, y, "01", max_demand_price) for x in es_scale_list for y in route_list]
    ...
    for k, v in result_df_dict.items():
        result_dir = Path(f"./data/{exp_name}/{k}/opt_result/")
        result_dir.mkdir(parents=True, exist_ok=True)
        v.to_csv(result_dir.joinpath(f"estimate_result_scale_all_optim.csv"))
