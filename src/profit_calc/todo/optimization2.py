...
from utils.time_process import  generate_day_pairs
...
class EsArbitraryRangeScheduler_withMaxDemand:
    ...
    def modeling2solving(self):
        ...
        # 目标函数
        profit = 31 * time_ratio * (e_c_in_agg_vec + e_c_out_agg_vec) @ p + self.max_demand_price * cp.min(e_c_in_agg_vec)
...
def one_process(es_scale, demand_load_df, ele_price_df):
    ...
    end_time = datetime(2025, 3, 1, 0, 0, 0)
    ...
    return es_scale, result_df




if __name__ == '__main__':
    print("start!", exp_name, "node:", node_name)
    mp_input_list = [(es_scale, demand_load_df, ele_price_df)for es_scale in range(50, 500, 50)]
    mp_result_list = []
    ...
    for result_i in mp_result_list:
        es_scale = result_i[0]
        result_i[1].to_csv(f"./data/{exp_name}/{node_name}/opt_result/es_scale_experiment/schedule_result_scale_{es_scale}.csv")