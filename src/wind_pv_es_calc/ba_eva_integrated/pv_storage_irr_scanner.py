...
从 ba_eva_optim_version/sichuang2.py 提取并泛化。
"""
...
from .multi_node_scanner import compute_irr
...
class PVStorageIRRConfig:
    """光储 IRR 扫描配置。"""
    # 光伏
    pv_capex_yuan: float = 2_300_000_000.0
    # 储能
    storage_capex_per_kwh: float = 800.0
    # 上网电价
...
class DeltaIRRRow:
    """相邻储能容量的 IRR 变化。"""
    buy_price_per_kwh: float = 0.0
    storage_from_mwh: float = 0.0
    storage_to_mwh: float = 0.0
    delta_irr_percent: float | None = None


@dataclass(slots=True)
class PVStorageIRRResult:
    """光储 IRR 扫描完整结果。"""
    scan_df: pd.DataFrame | None = None
    delta_df: pd.DataFrame | None = None
    best: PVStorageIRRRow | None = None
...
def simulate_annual_gain(
    ...
    cfg: PVStorageIRRConfig = PVStorageIRRConfig(),
) -> tuple[float, float]:
    ...
    for _, row in df.iterrows():
        ...
        Gain3 = storage_used * 1000 * buy_price_per_kwh

        # ③ 剩余 PV 上网（≤ max_export_ratio）
...
def _compute_single_irr(
    ...
    annual_gain, annual_energy = simulate_annual_gain(
        storage_mwh, buy_price_per_kwh, df,
    ...
    # 总 CAPEX
    storage_capex = storage_mwh * 1000 * cfg.storage_capex_per_kwh
    total_capex = cfg.pv_capex_yuan + storage_capex

    # IRR
    cashflows = [-total_capex] + [annual_cf] * cfg.life_years
    irr = compute_irr(cashflows)

    return PVStorageIRRRow(
...
# IRR 扫描
# ============================================================
def scan_pv_storage_irr(
    df: pd.DataFrame,
    storage_range: np.ndarray | list,
    ...
    if not rows:
        return PVStorageIRRResult()

    from dataclasses import asdict

    scan_df = pd.DataFrame([asdict(r) for r in rows])

    # ΔIRR：同一购电价下，相邻储能容量的 IRR 变化
    delta_rows: list[DeltaIRRRow] = []
    for bp in buy_price_range:
        sub = scan_df[scan_df["buy_price_per_kwh"] == bp].sort_values("storage_mwh")
        for i in range(len(sub) - 1):
            ...
            delta_rows.append(DeltaIRRRow(
                ...
                storage_from_mwh=s1["storage_mwh"],
                storage_to_mwh=s2["storage_mwh"],
