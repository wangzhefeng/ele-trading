...
def test_eta_roundtrip_dispatch():
    ...
    r1 = simulate_operation(load, wind_u, solar_u, 30, 20, 40, sp_rt)
    r2 = simulate_operation(load, wind_u, solar_u, 30, 20, 40, sp_sep)
...
def test_curve_based_energy_check():
    ...
    load, _, solar_u = _make_data(8760)
    result = curve_based_energy_check(load, solar_u, green_ratio_min=0.3, self_use_min=0.6)
    assert 'load_gwh_year' in result
