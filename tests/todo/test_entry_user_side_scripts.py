...
def test_run_user_side_bess_dispatch():
    result = _run_script("optimization/todo/run_user_side_bess_dispatch.py")
    assert result.returncode == 0, result.stderr[:300]


def test_run_user_side_pv_dispatch():
    result = _run_script("optimization/todo/run_user_side_pv_dispatch.py")
    assert result.returncode == 0, result.stderr[:300]


def test_run_user_side_pv_bess_dispatch():
    result = _run_script("optimization/todo/run_user_side_pv_bess_dispatch.py")
    assert result.returncode == 0, result.stderr[:300]
