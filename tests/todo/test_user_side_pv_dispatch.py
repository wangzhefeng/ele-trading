...
import ele_trading.optimization.todo as optimization
...
def test_pv_surplus_can_export_or_curtail_by_policy():
    allow_export = run_user_side_pv_dispatch(
        _pv_input(load_forecast=[3.0], pv_forecast=[8.0], allow_export=True)
    )
    no_export = run_user_side_pv_dispatch(
        _pv_input(load_forecast=[3.0], pv_forecast=[8.0], allow_export=False)
    )

    assert allow_export.pv_to_grid == pytest.approx([5.0])
    assert allow_export.pv_curtailment == pytest.approx([0.0])
    assert no_export.pv_to_grid == pytest.approx([0.0])
    assert no_export.pv_curtailment == pytest.approx([5.0])
...
def test_export_limit_sends_excess_to_curtailment():
    ...
    assert result.pv_to_grid == pytest.approx([4.0])
    assert result.pv_curtailment == pytest.approx([3.0])
...
def test_pv_dispatch_rejects_invalid_input():
    ...
    with pytest.raises(ValueError, match="pv_forecast must be non-negative"):
        run_user_side_pv_dispatch(
            _pv_input(load_forecast=[1.0], pv_forecast=[-1.0])
        )
    with pytest.raises(ValueError, match="step_hours must be positive"):
        invalid = _pv_input(load_forecast=[1.0], pv_forecast=[1.0])
