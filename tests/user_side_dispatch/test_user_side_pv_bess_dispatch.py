...
from ele_trading.user_side_dispatch.algorithms.user_side_renewable_bess_dispatch_class import (
...
def test_pv_bess_dispatch_energy_balance_and_soc():
    dispatch_input = _dispatch_input(
        ...
        renewable_forecast=[0.0, 8.0, 8.0, 0.0],
    ...
    for t in range(len(dispatch_input.timestamps)):
        assert (
            result.renewable_to_load[t]
            + result.renewable_to_bess[t]
            + result.renewable_to_grid[t]
            + result.renewable_curtailment[t]
        ) == pytest.approx(dispatch_input.renewable_forecast[t])
        assert (
            result.renewable_to_load[t]
        ...
        assert result.charge_power[t] == pytest.approx(
            result.renewable_to_bess[t] + result.grid_to_bess[t]
        )
    ...
    assert result.renewable_to_bess == pytest.approx(renewable_result.renewable_to_bess)
...
def test_demand_charge_reduces_peak_grid_import_with_bess():
    base = _dispatch_input(
        ...
        renewable_forecast=[0.0, 0.0, 0.0, 0.0],
        demand_charge_rate=0.0,
        initial_soc=6.0,
    )
...
def test_export_policy_controls_pv_to_grid():
    ...
    assert allow_export.renewable_to_grid == pytest.approx([2.0])
...
def test_optional_dispatch_policy_limits_windows():
    ...
    result = run_user_side_pv_bess_dispatch(
        _dispatch_input(
            ...
            renewable_forecast=[8.0, 8.0, 8.0, 0.0],
            allow_export=False,
            initial_soc=2.0,
            policy=policy,
        )
