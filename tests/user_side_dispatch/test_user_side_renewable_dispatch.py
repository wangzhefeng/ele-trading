...
from ele_trading.user_side_dispatch.adapters.dispatch_adapters import run_user_side_wind_dispatch
from ele_trading.user_side_dispatch.adapters.dispatch_adapters import (
...
def test_wind_dispatch_exports_surplus_and_limits_grid_import_peak():
    ...
    assert result.renewable_to_load == pytest.approx([3.0, 5.0, 8.0])
    assert result.renewable_to_grid == pytest.approx([0.0, 1.0, 1.0])
    assert result.renewable_curtailment == pytest.approx([0.0, 1.0, 1.0])
...
def test_wind_dispatch_curtails_surplus_when_export_is_disabled():
    ...
    assert result.renewable_to_load == pytest.approx([4.0])
    assert result.renewable_to_grid == pytest.approx([0.0])
    assert result.renewable_curtailment == pytest.approx([5.0])
...
def test_wind_bess_dispatch_charges_surplus_wind_and_later_discharges():
    ...
    assert result.renewable_to_bess[0] > 0.0
